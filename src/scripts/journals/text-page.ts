export default class AdventureJournalTextPageSheet extends foundry.applications.sheets.journal.JournalEntryPageProseMirrorSheet {
    protected _insertElement(element: HTMLElement): void {
        ...
        element.querySelectorAll('[data-id][data-type="Scene"]').forEach(scene => {
    ...
    #updateImages(element: HTMLElement) {
        element.querySelectorAll('img.outside').forEach((img: HTMLElement) => {
            const src = img.getAttribute('src');

            img.style.shapeOutside = `url('${src}')`;
        });
    }
    ...
    #updateSounds(element: HTMLElement) {
        element.querySelectorAll('[data-id][data-type="AmbientSound"]').forEach((link: HTMLElement) => {
